...
var User = exports.User = function () {
	function User(user_id, purchases) {
	...
	_createClass(Split, [{
		...
		value: function getOweString() {
			...
			return Object.keys(this.users).map(function (user) {
				...
				if (userTotal <= userShare) return user + " owes: " + (userShare - userTotal).toFixed(2);else return user + " is owed: " + (userTotal - userShare).toFixed(2);
			}).join("\n");
		}
